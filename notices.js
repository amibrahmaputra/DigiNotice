...
router.get(
	...
	async (req, res) => {
		...
		const { q = '', category = '', department = '', approved = '1' } = req.query;
...
router.post(
	...
	async (req, res) => {
		...
		const result = await run(
			'INSERT INTO notices (title, content, category, department, author_id, approved, comment_enabled, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?)',
			[title, content, category, department, req.user.id, approved, comment_enabled ? 1 : 0, now, now]
		);
		...
		if (approved === 1) {
			...
			console.log(`📝 Notice ${noticeId} saved as draft (pending admin approval)`);
		}
...
router.post('/:id/approve', requireAuth, requireRole('admin'), async (req, res) => {
	...
	// Only send emails if notice is being approved (not already approved)
	const wasApproved = existing.approved === 1;
	
	await run('UPDATE notices SET approved = 1, updated_at = ? WHERE id = ?', [new Date().toISOString(), id]);
	const row = await get('SELECT * FROM notices WHERE id = ?', [id]);
	console.log(`✅ Notice ${id} approved successfully`);
	
	// Send email notifications only if notice was not previously approved
	if (!wasApproved) {
